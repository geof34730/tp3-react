...
const Term = (props) => {
    ...
    const onClickDeleteTerm = useContext(MemoContext).handleClickDeleteTerm;
    const onClickEditTerm = useContext(MemoContext).handleClickEditTerm;
    const onClickTerm = useContext(MemoContext).handleClickTerm;
    ...
    return (
      <div className="d-flex btn-group"  role="group" aria-label="Basic example" >
          <button
              className="btn btn-secondary ms-3 px-4 text-uppercase"
              onClick={() => {onClickTerm(props.term)}}
                >
              {props.term.name}
            </button>

          {props.editMode && (
              <button type="button"  className="btn btn-outline-success" onClick={() => {onClickEditTerm(props.indexTerm)}}>
